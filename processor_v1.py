...
class PBmapper():
    ...
    def modifier(self, company_df, pricing_df):
        ...
        company_df.to_excel("treated_p21_items.xlsx", index = False)
        pricing_df.to_excel("treated_controlstop_items.xlsx", index = False)
    ...
    def matching_logic(self, company_df, pricing_df):
        ...
        # Merging the dataframes on 'Stripped_supplier_PN' (assuming this is the column to match)
        merged_df = pd.merge(company_df, pricing_df, 
                            left_on='Stripped_supplier_PN',  # Match this column from company_df
                            right_on='Stripped_supplier_PN', # Match this column from pricing_df
                            how='left', 
                            suffixes=('_company', '_pricing'))

        # Correct column names after the merge:
        # 'Stripped_supplier_PN_company' (from company_df)
        # 'Stripped_supplier_PN_pricing' (from pricing_df)

        # If there's a match, update the Matching_status and add the Matched_companydoc_SPN
        merged_df['Matching_status'] = merged_df['Stripped_supplier_PN_pricing'].apply(
            lambda x: 'Matched' if pd.notnull(x) else 'Not Matched'
        )

        # Set 'Matched_companydoc_SPN' based on the matching supplier part number in the pricing_df
        merged_df['Matched_companydoc_SPN'] = merged_df['Supplier_part_no_pricing'].where(
            pd.notnull(merged_df['Stripped_supplier_PN_pricing']), ''
        )

        # Update the original company_df from merged_df
        company_df['Matching_status'] = merged_df['Matching_status']
        company_df['Matched_companydoc_SPN'] = merged_df['Matched_companydoc_SPN']

        # If you need to update the pricing_df as well (for the matched supplier part numbers)
        pricing_df['Matching_status'] = merged_df['Matching_status']
        pricing_df['Matched_pricingdoc_SPN'] = merged_df['Supplier_part_no_company'].where(
            pd.notnull(merged_df['Stripped_supplier_PN_pricing']), ''
        )
...
if __name__ == "__main__":
    ...
    #controlstopfiles = controlstopfiles[controlstopfiles["Matching_status"] == "Not Matched"]
    ...
    P21_files.to_excel("v1_p21_matched_merged.xlsx", index = False)
    controlstopfiles.to_excel("v1_controlstop_matched_merged.xlsx", index = False)
